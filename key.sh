...
# 参数写入：
# - bantime.increment / bantime.factor / bantime.maxtime 属于 [DEFAULT] 段
# - 其余参数写入 [sshd] 段
set_f2b_conf() {
    local key=$1 val=$2
    local target_section="$TARGET_JAIL"
    case "$key" in
        bantime.increment|bantime.factor|bantime.maxtime)
