...
SSHD_CUSTOM="${SSHD_CONF_DIR}/99-key.sh.conf"
...
# ============ SSH 配置管理 ============
# 读取顺序：优先读 99-key.sh.conf（我们写入的地方）→ 主文件 → 其他 .d 文件
# 与 OpenSSH "First Match Wins" 生效顺序一致
get_sshd_config_val() {
    local key="$1" default_val="$2"
    local val=""

    # 1. 优先读取我们写入的 99-key.sh.conf
...
# 写入 SSH 配置
# 优先写入 99-key.sh.conf（避开 Match 块陷阱 + 最高优先级）
# 无 sshd_config.d 目录时才写主文件
set_sshd_config() {
    local param="$1" value="$2"

    if [ -d "$SSHD_CONF_DIR" ]; then
        # 1. 注释其他 .conf 中的同参数项，避免 First Match Wins 冲突
        for conf in "${SSHD_CONF_DIR}"/*.conf; do
            [ -f "$conf" ] || continue
            [ "$conf" = "$SSHD_CUSTOM" ] && continue
...
append_key_with_meta() {
    ...
    while IFS= read -r line; do
        ...
        # 从行首保留到 base64 主体结束（含选项前缀和密钥类型），丢弃之后的内容
        local core_key
        core_key=$(echo "$line" | awk '{
            for(i=1;i<=NF;i++){
                if($i ~ /^AAAA/){
                    for(j=1;j<=i;j++) printf "%s%s", $j, (j<i?" ":"")
                    print ""
                    exit
                }
            }
...
check_f2b_install() {
    ...
    if [ ! -d "/etc/fail2ban" ]; then
        echo -e "${ERROR} /etc/fail2ban 目录不存在，请重新安装 Fail2Ban。${RESET}"
        read -rp "按回车键返回..."
        return 1
    fi
